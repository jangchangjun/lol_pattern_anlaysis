...
이에 추가로 게임의 승리를 결정짓는 것은 결국 전투와 운영을 통한 상대와의 격차 차이가 가장 큰 요인이라 판단해 gap데이터로 정확도를 계산하고자 하였다.
...
gap at/af14와 y 간의 상관계수를 추출한 후, 상관계수가 절댓값 0.1 이하인 데이터는 승리 예측에 큰 영향을 미치지 않는다고 판단하여 삭제하였다.

그 뒤, 선택된 데이터에 대해 Min-Max Scaler를 이용해 정규화를 진행한 후, 승/패 예측을 위한 이진 분류 문제에 적합한 로지스틱 회귀 모델을 사용하여 학습을 진행하였다.
...
모델의 정확도는 0.93 모델의 성능을 표현하는 auroc값은 0.978로 높은 성능을 보였다.

### 4. 결론 및 느낀점

본 프로젝트에서는 전투, 운영, 격차 지표와 같은 게임의 승리를 판가름하는 주요 데이터를 활용하여 승패 예측을 진행하였다. 데이터 가공 후 정확도를 예측하는 과정을 홀로 수행하면서, 프로젝트의 방향성과 과정을 설정하는 데 많은 배움을 얻었다. 중간중간 발생하는 문제들을 해결하는 과정에선 성취감 또한 느낄 수 있었다.

이 프로젝트를 끝으로 하지 않고, 향후 프로젝트를 더욱 발전시켜 나가고자 한다. 사소한 데이터라도 최대한 수집하여 보다 정교한 예측 모델을 구축해보고 싶다. 이를 통해 승패 예측의 정확성을 높이고, 실제 게임 환경에서의 적용 가능성을 탐색하고자한다.
