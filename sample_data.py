...
def extract_data(df_match, df_timeline, gamer_name, opposite=False):
    ...
    for j, match in enumerate(df_match['info'].values):
        if pd.isna(df_match['metadata'][j]):
            print(f"no gain in {j+1}th math")
            continue
        ...
        for k, timeline in enumerate(df_timeline['metadata']):
            ...
            if not timelineExist:
                print(f"{matchId} does nor exist in timeline data")
                continue
        ...
        if check_gamer_mid and match['gameDuration'] > 1200:
            ...
            target_data["opponentId"] = match_o['participantId']
...
opppnent_data = extract_data(df_match, df_timeline, gamer_name, opposite=True)
...
with open('extracted_test_opponent_data.json', 'w', encoding='utf-8') as f:
    json.dump(opppnent_data, f, ensure_ascii=False, indent=4)